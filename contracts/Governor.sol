...
contract Governor {
    ...
    string public constant name = "eMax Governor";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public pure returns (uint) { return 400000e18; } // 400,000 = 4% of NFTMax

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public pure returns (uint) { return 100000e18; } // 100,000 = 1% of NFTMax
    ...
    /// @notice The address of the NFTMax governance token
    NFTMaxInterface public nftmax;
    ...
    constructor(address timelock_, address nftmax_, address guardian_) public {
        timelock = TimelockInterface(timelock_);
        nftmax = NFTMaxInterface(nftmax_);
        guardian = guardian_;
    }

    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        require(nftmax.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "Governor::propose: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) public {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || nftmax.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "Governor::cancel: proposer above threshold");
    ...
    function _castVote(address voter, uint proposalId, bool support) internal {
        ...
        uint96 votes = nftmax.getPriorVotes(voter, proposal.startBlock);
