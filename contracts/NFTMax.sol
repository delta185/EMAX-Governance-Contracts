...
contract NFTMax {
    ...
    string public constant name = "NFTMax";

    /// @notice EIP-20 token symbol for this token
    string public constant symbol = "NFTMAX";

    /// @notice EIP-20 token decimals for this token
    uint8 public constant decimals = 18;

    /// @notice Total number of tokens in circulation
    uint public constant totalSupply = 10000000e18; // 10 million NFTMax
    ...
    /**
     * @notice Construct a new NFTMax token
    ...
    function approve(address spender, uint rawAmount) external returns (bool) {
        ...
        if (rawAmount == uint(-1)) {
            ...
            amount = safe96(rawAmount, "NFTMax::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint rawAmount) external returns (bool) {
        uint96 amount = safe96(rawAmount, "NFTMax::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external returns (bool) {
        ...
        uint96 amount = safe96(rawAmount, "NFTMax::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != uint96(-1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "NFTMax::transferFrom: transfer amount exceeds spender allowance");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(signatory != address(0), "NFTMax::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "NFTMax::delegateBySig: invalid nonce");
        require(now <= expiry, "NFTMax::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "NFTMax::getPriorVotes: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "NFTMax::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "NFTMax::_transferTokens: cannot transfer to the zero address");

        balances[src] = sub96(balances[src], amount, "NFTMax::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "NFTMax::_transferTokens: transfer amount overflows");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "NFTMax::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "NFTMax::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "NFTMax::_writeCheckpoint: block number exceeds 32 bits");
